...
def main():
    ...
    es_formula = input("Enter a chemical formula to compute molar es_mass: ")
    
    # Get the mass of a chemical sample in grams from the user.
    es_mass = float(input("Enter the es_mass of the sample (in grams): "))
    
    # Call the make_periodic_table and make_known_molecule_list function and
    # store the periodic table and known molecule lists in a variable.
    es_periodic_table_dict = make_periodic_table()
    es_known_molecules = make_known_molecule_list()
    
    # Call the parse_formula function to convert the
    # chemical formula given by the user to a compound
    # list that stores element symbols and the quantity
    # of atoms of each element in the molecule.
    element_symbols = parse_formula(es_formula, es_periodic_table_dict)
    
    # Call the compute_molar_mass function to compute the
    # molar es_mass of the molecule from the compound list.
    es_molar_mass = compute_molar_mass(element_symbols, es_periodic_table_dict)

    # Compute the number of moles in the sample.
    es_moles = es_mass / es_molar_mass

    # Find if the molecule entered is on the 
    # known molecule list
    es_formula_name = get_formula_name(es_formula, es_known_molecules)
    
    # Print the molar es_mass and molecule name.
    print(f"Molecule: {es_formula_name}")
    print(f"Molar es_mass: {es_molar_mass:.5f} grams/mole")

    # Print the number of moles.
    print(f"Moles: {es_moles:.5f} moles")
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    """Compute and return the total molar es_mass of all the
    ...
    Parameters
        ...
        periodic_table_dict is the compound dictionary returned
            from make_periodic_table.
        Return: the total molar es_mass of all the elements in
    ...
    # For each list in the compound symbol_quantity_list:
        ...
        # Get the atomic es_mass for the symbol from the dictionary.
        # Multiply the atomic es_mass by the quantity.
        # Add the product into the total molar es_mass.
    es_molar_mass = 0
    for element, quantity in symbol_quantity_list:
        es_element_info = periodic_table_dict[element]
        es_atomic_mass = es_element_info[ATOMIC_MASS_INDEX]
        es_molar_mass += (es_atomic_mass * quantity)
    
    # Return the total molar es_mass.
    return es_molar_mass



def make_periodic_table():
    periodic_table_list = {
        # Symbol: [Name, Atomic es_mass]
        "Ac": ["Actinium",227],
        "Ag": ["Silver",107.8682],
